...
ManagerItemT = TypeVar("ManagerItemT")
ListUpdateContextT = TypeVar("ListUpdateContextT")
...
class ManagerUiLifecycleMixin:
    """UI setup, status, preview refresh, and teardown behavior."""
...
class ManagerActionWorkflowMixin:
    """Action dispatch, CRUD actions, and edited-code workflow behavior."""
    ...
    def _action_operations(self) -> ManagerActionOperations:
        ...
        if self.ACTION_REGISTRY or self.DYNAMIC_ACTIONS:
            raise NotImplementedError(
                "String-based ACTION_REGISTRY/DYNAMIC_ACTIONS are no longer supported; "
                "override _action_handlers() and _dynamic_action_handlers() with bound callables."
            )

        return ManagerActionOperations(
            widget=self,
            action_handlers=self._action_handlers(),
            dynamic_action_handlers=self._dynamic_action_handlers(),
    ...
    def _action_handlers(self) -> Dict[str, ManagerActionHandler]:
        """Return bound action handlers keyed by button action id."""
        return {}

    def _dynamic_action_handlers(self) -> Dict[str, ManagerDynamicActionHandler]:
        """Return bound dynamic action resolvers keyed by button action id."""
        return {}
...
class ManagerListUpdateWorkflowMixin:
    """List update operation construction and display-building behavior."""
...
class AbstractManagerWidget(
    ManagerUiLifecycleMixin,
    ManagerActionWorkflowMixin,
    ManagerSelectionWorkflowMixin,
    ManagerListUpdateWorkflowMixin,
    QWidget,
    CrossWindowPreviewMixin,
    FlashMixin,
    ...
    SELECTION_CLEARED_PAYLOAD: ClassVar[str | None] = None
    SCOPE_ITEM_TYPE: ClassVar[ListItemType | None] = None
    ...
    def __init__(self, service_adapter, color_scheme=None, parent=None):
        ...
        if service_adapter is None:
            raise ValueError("AbstractManagerWidget requires a service_adapter.")

        if color_scheme is None:
            color_scheme = service_adapter.get_current_color_scheme()
    ...
    @abstractmethod
    def show_item_editor(self, item: ManagerItemT) -> None:
        """
    ...
    def _format_item_content(
        ...
        item: ManagerItemT,
        index: int,
        context: ListUpdateContextT | None,
    ) -> StyledText | str:
    ...
    @abstractmethod
    def _get_list_item_tooltip(self, item: ManagerItemT) -> str:
        """
    ...
    def _get_list_placeholder(self) -> Optional[Tuple[str, str | None]]:
        """
    ...
    def prepare_list_update(self) -> ListUpdateContextT | None:
        """
    ...
    @abstractmethod
    def _get_scope_for_item(self, item: ManagerItemT) -> str:
